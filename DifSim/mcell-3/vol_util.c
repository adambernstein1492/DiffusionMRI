...
#ifdef MRI_DIFF_SIM
    if (world->is_in_region) 
    {
      if (! world->is_in_region(m)) continue;
    }
#endif
...
#ifdef MRI_DIFF_SIM
    m->orig_pos.x = m->pos.x;
    m->orig_pos.y = m->pos.y;
    m->orig_pos.z = m->pos.z;
    m->offset.x = 0.0;
    m->offset.y = 0.0;
    m->offset.z = 0.0;
    m->phase_shift = calloc(world->nDirs, sizeof(double));
    m->Mx = calloc(world->nDirs, sizeof(double));
    m->My = calloc(world->nDirs, sizeof(double));
    m->Mz = calloc(world->nDirs, sizeof(double)); 
    for (int j = 0; j < world->nDirs; j++) m->Mz[j] = 1.;
#endif

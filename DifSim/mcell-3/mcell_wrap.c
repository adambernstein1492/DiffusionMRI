...
void mcell_iteration(long long it_time)
{
  ...
  /* The last iteration is a half-iteration. */
...
void mcell_wrap_init(char *file_name, int nDirs, int seed, char *log_file)
{
  ...
  if (log_file) {
    if ((lf= fopen(log_file, "w")) == NULL)
    {
      return;
    } else {
      mcell_set_log_file(lf);
      mcell_set_error_file(lf);
    }
  } else {
  ...
  world->log_freq = ULONG_MAX;
  world->seed_seq = seed;
  world->mdl_infile_name = strdup(file_name);
...
{
  return collide_wall(start,move,face,t,hitpt, update_move);
}

double wrap_rng() {
  return rng_dbl(world->rng);
}

int bisect_wrap(double *list,int n,double val) {
  return bisect(list,n,val);
}

double collide_sv_time_wrap(struct vector3 *here,struct vector3 *move,struct subvolume *sv) {
  return collide_sv_time(here,move,sv);
}

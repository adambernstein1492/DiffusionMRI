...
struct species
{
  ...
  double lambda_1, lambda_2, lambda_3; /*eigenvalues of diffusion tensor */
  ...
  struct name_orient *period_mols;  /* names of the mols that are PERIODIC for surface */
  ...
  double relax_t1;                   /* This species T1 relaxation constant -- really should be associated with tissue
                                     but no elegant way to declare tissue constants and check them at this point */
  double relax_t2;                      /* This species's T2 relaxation constant */
};
...
#ifdef MRI_DIFF_SIM 
  struct vector3 orig_pos;        /* Position in space when released (translated when crossing periodic boundaries) */
  struct vector3 offset;          /* Translation necessary to get molecule back to its original reference frame */
  double *phase_shift;            /* This molecule's phase shift */
  double *Mx;                     /* Magnetization in Cartesian coordinates */
  double *My;                     
  double *Mz;
#endif /* MRI_DIFF_SIM */
...
#ifdef MRI_DIFF_SIM	
  int nDirs;
  int (*is_in_region)(struct volume_molecule *mol);
  void *msim;
#endif /* MRI_DIFF_SIM */

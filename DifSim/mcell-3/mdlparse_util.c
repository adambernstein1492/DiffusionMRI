                                         ...
                                         double lambda_3)
{
  ...
  /*MRI*/
  specp->lambda_1 = lambda_1;
  specp->lambda_2 = lambda_2;
  specp->lambda_3 = lambda_3;
  specp->relax_t1 = rel_t1;
  specp->relax_t2 = rel_t2;
  /*MRI*/     
...
/**************************************************************************
 ...
 In: mpvp: parser state
     reaction_type: RFLCT, PERIOD, TRANSP, or SINK
                                          ...
                                          short orient)
{
  ...
  switch (reaction_type)
  {
    case PRD:
      ...
      if(surface_class->period_mols == NULL)
      {
         no->next = NULL;
         surface_class->period_mols = no;
      }else{
         no->next = surface_class->period_mols;
         surface_class->period_mols = no;
      }
      break;
    case RFLCT:
...
static struct rxn *split_reaction(struct mdlparse_vars *mpvp, struct rxn *rx)
{
  ...
  while (to_place != NULL)
  {
    if (to_place->flags & (PATHW_PERIOD | PATHW_TRANSP | PATHW_REFLEC | PATHW_ABSORP | PATHW_CLAMP_CONC))
    {
    ...
    else
    {
      ...
      {
        if (curr_rxn_ptr->pathway_head->flags & (PATHW_PERIOD | PATHW_TRANSP | PATHW_REFLEC | PATHW_ABSORP))
          continue;
  ...
  for (int n_rxn_bin=0; n_rxn_bin<mpvp->vol->rxn_sym_table->n_bins; n_rxn_bin++)
  {
         ...
         sym = sym->next )
    {
      ...
      for (path=reaction->pathway_head ; path != NULL ; path = path->next)
      {
        /* if it is a special reaction - check for the duplicates pathways */
           if (path->next != NULL)
           {
              if ((path->flags & PATHW_PERIOD) && (path->next->flags & PATHW_PERIOD))
              {
                if((path->orientation2 == path->next->orientation2) ||
                   (path->orientation2 == 0) || (path->next->orientation2 == 0))
                { 
                   mcell_error("Exact duplicates of special reaction PERIODIC = %s are not allowed.  Please verify the contents of DEFINE_SURFACE_CLASS statement.", path->reactant2->sym->name);
                }
              }
	...
	/* Any reactants that don't appear are set to be destroyed. */
        ...
        for (int n_pathway=0; path!=NULL ; n_pathway++ , path = path->next)
        {
          ...
          else if ((path->flags & PATHW_PERIOD) != 0) {
            rx->n_pathways = RX_PERIOD;

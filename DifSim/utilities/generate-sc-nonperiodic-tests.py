...
bigDeltaEff = float(sys.argv[3])
bigDelta = bigDeltaEff + littleDelta/3.0
timeStep = 10
if len(sys.argv) > 4:
...
# bigDeltaEff: big delta (pulse separation), units of microseconds,
#  but bigDeltaEff = bigDelta - littleDelta/3
#  so bigDelta = bigDeltaEff + littleDelta/3
#
...
nparts = 2500000  # number of diffusing molecules
...
for i in range(46):
    f = open('sc-%d.txt' % (i*2), 'w')
    f.write(standardValues % (b, bigDelta - littleDelta, littleDelta, gradientStrength*math.cos(i*math.pi/90), timeStep))
    f.close()
